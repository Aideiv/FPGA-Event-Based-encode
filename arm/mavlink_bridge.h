...
class MAVLinkBridge {
   public:
    ...
    explicit MAVLinkBridge(const Config& cfg, SendCallback send_fn, RecvCallback recv_fn)
        : cfg_(cfg),
          send_(send_fn),
          recv_(recv_fn),
          last_heartbeat_ms_(0),
    ...
    // ------------------------------------------------------------------
    bool send_velocity_setpoint(const FlightCommand& cmd, uint32_t current_time_ms) {
        ...
        mavlink_msg_set_position_target_local_ned_pack(
            ...
            cfg_.target_system_id, cfg_.target_component_id, cfg_.velocity_frame, type_mask, 0, 0,
            0,                       // position x, y, z (ignored)
        ...
        uint16_t len = pack_set_position_target_local_ned_manual(
            buf, current_time_ms, type_mask, vx_ned, vy_ned, vz_ned, cmd.yaw_rate);
#endif
    ...
    bool send_arm_command(bool arm) {
        ...
        mavlink_msg_command_long_pack(cfg_.system_id, cfg_.component_id, &msg,
    ...
    // ------------------------------------------------------------------
    const VehicleState& get_vehicle_state() const { return vehicle_state_; }
   ...
   private:
    ...
    void send_heartbeat(uint32_t current_time_ms) {
        ...
        mavlink_msg_heartbeat_pack(cfg_.system_id, cfg_.component_id, &msg,
        ...
        // Payload: type (1B), autopilot (1B), base_mode (1B), custom_mode (4B), system_status (1B),
        // mavlink_version (1B)
    ...
    // ------------------------------------------------------------------
    uint16_t pack_set_position_target_local_ned_manual(uint8_t* buf, uint32_t time_boot_ms,
                                                       uint16_t type_mask, float vx, float vy,
                                                       float vz, float yaw_rate) {
        ...
        // type_mask (uint16)
        memcpy(buf + off, &type_mask, 2);
        off += 2;

        // position x, y, z (float32 each — ignored)
        float zero = 0.0f;
        memcpy(buf + off, &zero, 4);
        off += 4;
        memcpy(buf + off, &zero, 4);
        off += 4;
        memcpy(buf + off, &zero, 4);
