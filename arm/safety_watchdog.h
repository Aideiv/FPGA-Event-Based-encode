...
class SafetyWatchdog {
   public:
    ...
    explicit SafetyWatchdog(const Config& cfg)
        : cfg_(cfg),
          armed_(false),
          last_valid_timestamp_(0),
          smoothed_vx_(0),
    ...
    // ------------------------------------------------------------------
    EvasionCommand apply_safety(const EvasionCommand& cmd, uint32_t current_timestamp_ms,
    ...
    // ------------------------------------------------------------------
    void update_altitude(float altitude_m) { altitude_m_ = altitude_m; }
   ...
   private:
    ...
    SafetyStatus check_safety(const EvasionCommand& cmd, uint32_t current_timestamp_ms,
                              float current_altitude_m) {
        ...
        if (std::isnan(cmd.velocity_x) || std::isinf(cmd.velocity_x) ||
            ...
            std::isnan(cmd.velocity_z) || std::isinf(cmd.velocity_z) || std::isnan(cmd.yaw_rate) ||
            std::isinf(cmd.yaw_rate)) {
        ...
        status.watchdog_timer_ms = static_cast<float>(current_timestamp_ms - last_valid_timestamp_);
