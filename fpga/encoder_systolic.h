...
// ---------------------------------------------------------------------------
void tile_matmul(coord_enc_t events[3],  // Single event: [t, x, y]
...
// ---------------------------------------------------------------------------
void local_geometry_encoder(coord_enc_t events_txy[MAX_EVENTS][3], event_cnt_t num_events,
                            knn_output_t knn_results[MAX_EVENTS], encoder_weights_t weights,
...
#pragma HLS UNROLL factor = 8
            ...
            if (denom > prod_t(0.001)) {
                enc_out_t g_re = enc_out_t((G_real[i][d] * prod_t(epA_real[i][d]) +
                                            G_imag[i][d] * prod_t(epA_imag[i][d])) /
                                           denom);
                enc_out_t g_im = enc_out_t((G_imag[i][d] * prod_t(epA_real[i][d]) -
                                            G_real[i][d] * prod_t(epA_imag[i][d])) /
                                           denom);
                sum_real += g_re;
                sum_imag += g_im;
            }
