...
template<int W, int I> class ap_ufixed;
...
public:
    ...
    template<int W2> ap_uint(const ap_int<W2>& o) : val(static_cast<uint64_t>(o.val) & MASK) {}
    ...
    ap_uint operator++(int) { ap_uint t(*this); val = (val + 1) & MASK; return t; }
    ap_uint& operator--() { val = (val - 1) & MASK; return *this; }
    ap_uint operator--(int) { ap_uint t(*this); val = (val - 1) & MASK; return t; }
    ap_uint& operator+=(uint64_t v) { val = (val + v) & MASK; return *this; }
    ap_uint& operator-=(uint64_t v) { val = (val - v) & MASK; return *this; }
...
public:
    ...
    template<int W2> ap_int(const ap_int<W2>& o) : val(clamp(o.val)) {}
    template<int W2> ap_int(const ap_uint<W2>& o) : val(clamp(static_cast<int64_t>(o.val))) {}
    template<int W2, int I2> ap_int(const ap_fixed<W2, I2>& f) : ap_int(static_cast<int64_t>(static_cast<float>(f))) {}
    ...
    ap_int& operator++() { val = clamp(val + 1); return *this; }
    ap_int operator++(int) { ap_int t(*this); val = clamp(val + 1); return t; }
    ap_int& operator--() { val = clamp(val - 1); return *this; }
    ap_int operator--(int) { ap_int t(*this); val = clamp(val - 1); return t; }
    ap_int& operator+=(int64_t v) { val = clamp(val + v); return *this; }
    ap_int& operator-=(int64_t v) { val = clamp(val - v); return *this; }
};
...
public:
    ...
    template<int W2, int I2> ap_fixed(const ap_fixed<W2, I2>& o) : ap_fixed(static_cast<float>(o)) {}
    template<int W2, int I2> ap_fixed(const ap_ufixed<W2, I2>& o) : ap_fixed(static_cast<float>(o)) {}
    template<int W2> ap_fixed(const ap_uint<W2>& o) : ap_fixed(static_cast<float>(o.to_uint64())) {}
...
public:
    ...
    template<int W2, int I2> ap_ufixed(const ap_ufixed<W2, I2>& o) : ap_ufixed(static_cast<float>(o)) {}
    operator float()  const { return static_cast<float>(val_) / SCALE; }
