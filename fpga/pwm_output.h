...
// ---------------------------------------------------------------------------
void pwm_output(velocity_t vx, velocity_t vy, velocity_t vz, velocity_t yaw_rate,
                ap_uint<1> enable_motors, motor_outputs_t& motors) {
...
#pragma HLS PIPELINE II = 1
    ...
    // M1 (FR): +vx -vy +vz +yaw
    thrust_t t1 = base_thrust + thrust_t(vx) * mix_vx_positive - thrust_t(vy) * mix_vy_positive +
                  thrust_t(vz) * mix_vz_positive + thrust_t(yaw_rate) * mix_yaw_pos;

    // M2 (FL): +vx +vy +vz -yaw
    thrust_t t2 = base_thrust + thrust_t(vx) * mix_vx_positive + thrust_t(vy) * mix_vy_positive +
                  thrust_t(vz) * mix_vz_positive - thrust_t(yaw_rate) * mix_yaw_pos;

    // M3 (RR): -vx +vy +vz +yaw
    thrust_t t3 = base_thrust - thrust_t(vx) * mix_vx_positive + thrust_t(vy) * mix_vy_positive +
                  thrust_t(vz) * mix_vz_positive + thrust_t(yaw_rate) * mix_yaw_pos;

    // M4 (RL): -vx -vy +vz -yaw
    thrust_t t4 = base_thrust - thrust_t(vx) * mix_vx_positive - thrust_t(vy) * mix_vy_positive +
                  thrust_t(vz) * mix_vz_positive - thrust_t(yaw_rate) * mix_yaw_pos;
