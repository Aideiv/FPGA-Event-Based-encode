...
#define ASSERT(cond, ...) do { if (!(cond)) FAIL(__VA_ARGS__); else PASS_count++; } while(0)
...
void feed_events_to_pipeline(
    control_regs_t& ctrl, aer_bus_t& aer_bus, ap_uint<64>& timestamp,
...
void test_pwm_output_basic() {
    ...
    ASSERT(motors_disarmed.m1 == PWM_MIN_TICKS && motors_disarmed.m2 == PWM_MIN_TICKS &&
           ...
           "disarmed motors should be PWM_MIN_TICKS, got m1=%lu", static_cast<unsigned long>(motors_disarmed.m1.val));
}
...
void test_spatial_hash() {
    ...
    for (int i = 0; i < static_cast<int>(count.val); i++) {
        if (knn_results[i].count.val > 0) { has_edges = true; break; }
    }
...
void test_ring_buffer() {
    ...
    for (int i = 0; i < 100; i++) {
        ring_buf[static_cast<int>(wr_ptr.val)].x = ap_ufixed<16,4>(static_cast<float>(i % 640) / 640.0f);
        ring_buf[static_cast<int>(wr_ptr.val)].y = ap_ufixed<16,4>(static_cast<float>((i*2) % 480) / 480.0f);
    ...
    uint64_t read_offset = (wr_ptr.val >= count.val) ? (wr_ptr.val - count.val) : (wr_ptr.val + RING_BUFFER_SIZE - count.val);
    ap_uint<12> read_ptr(read_offset);
    float first_x = static_cast<float>(ring_buf[static_cast<int>(read_ptr.val)].x);
    ASSERT(first_x >= 0.0f && first_x <= 1.0f, "first event x should be valid, got %.3f", static_cast<double>(first_x));
    
    TEST("Ring buffer wraps correctly");
    for (int i = 0; i < RING_BUFFER_SIZE - 50; i++) {
        ring_buf[static_cast<int>(wr_ptr.val)].x = ap_ufixed<16,4>(static_cast<float>((i+1000) % 640) / 640.0f);
