...
void test_looming_object_detected() {
    ...
    if (!result.objects.empty()) {
        CHECK(result.objects[0].ttc < 100.0f, 
              "ttc should be finite: %.2f", result.objects[0].ttc);
    }
    ...
    printf("    Clusters: %zu, max_urgency: %.3f, ttc: %.2fs\n",
           result.objects.size(), result.max_urgency, result.time_to_first_collision);
}
...
void test_lateral_motion_no_false_alarm() {
    ...
    CHECK(result.max_urgency < 0.9f, 
          "urgency should not be critical for lateral motion: %.3f", result.max_urgency);
}
...
void test_random_noise_low_urgency() {
    ...
    CHECK(result.max_urgency < 0.5f, 
          "random noise should have low urgency: %.3f", result.max_urgency);
    
    printf("    Threat: %s, urgency: %.3f\n",
           result.threat_detected ? "YES" : "NO", result.max_urgency);
}
...
void test_multi_object_clustering() {
    ...
    CHECK(result.objects.size() >= 2, 
          "expected >= 2 clusters, got %zu", result.objects.size());
    ...
    for (size_t i = 0; i < result.objects.size(); i++) {
        printf("    Cluster %zu: center=(%.3f,%.3f) extent=%.3f ttc=%.1fs urgency=%.2f events=%u\n",
               i,
               result.objects[i].center_x, result.objects[i].center_y,
               result.objects[i].spatial_extent,
               result.objects[i].ttc,
               result.objects[i].collision_urgency,
               result.objects[i].event_count);
    }
}
...
void test_safe_bearing() {
    ...
    printf("    Safe bearing: %.1f° (confidence: %.2f)\n",
           result.safe_bearing * 180.0f / M_PI, result.safe_bearing_confidence);
}
...
void test_evasion_controller_levels() {
    ...
    // Test: No threat → NONE level
    {
        ...
        TEST("No threat → NONE level");
        CHECK(cmd.level == EvasionLevel::NONE, 
              "expected NONE, got %s", evasion_level_name(cmd.level));
    ...
    // Test: High urgency → EMERGENCY level
    {
        ...
        TEST("High urgency → EMERGENCY level");
        CHECK(cmd.level == EvasionLevel::EMERGENCY, 
              "expected EMERGENCY, got %s", evasion_level_name(cmd.level));
...
void test_evasion_hysteresis() {
    ...
    ObjectCluster obj;
    obj.center_x = 0.3f; obj.center_y = 0.5f;
    obj.spatial_extent = 0.1f;
    obj.ttc = 0.4f;
    ...
    CHECK(last_level == EvasionLevel::NONE,
          "should reach NONE after sustained safety, got %s", 
...
void test_evasion_output_range() {
    ...
    CHECK(horiz <= cfg.max_horizontal_velocity * 1.01f,
          "horizontal velocity %.2f exceeds max %.2f", horiz, cfg.max_horizontal_velocity);
    
    TEST("Vertical velocity within max limit");
    CHECK(cmd.velocity_z >= 0.0f && cmd.velocity_z <= cfg.max_vertical_velocity * 1.01f,
          "vertical velocity %.2f out of range [0, %.2f]", 
          cmd.velocity_z, cfg.max_vertical_velocity);
    
    TEST("Yaw rate within max limit");
    CHECK(std::abs(cmd.yaw_rate) <= cfg.max_yaw_rate * 1.01f,
          "yaw rate %.2f exceeds max %.2f", cmd.yaw_rate, cfg.max_yaw_rate);
    
    printf("    cmd: vx=%.2f vy=%.2f vz=%.2f yaw=%.2f |horiz|=%.2f\n",
           cmd.velocity_x, cmd.velocity_y, cmd.velocity_z, cmd.yaw_rate, horiz);
}
